...
const CITUM_CORE_PATH = process.env.CITUM_CORE_PATH || path.resolve(PROJECT_ROOT, "..", "citum-core");
...
async function getFixtureData(type: string = "expanded") {
    ...
    try {
        ...
        console.error(`[Fixture] Failed to load ${fileName} from ${filePath}. Previews will be empty.`, e);
...
app.get("/styles", authMiddleware, async (c) => {
    ...
    try {
        const styles = await sql`SELECT * FROM styles WHERE user_id = ${userId} ORDER BY updated_at DESC`;
        return c.json(styles);
...
app.get("/auth/github/callback", async (c) => {
    ...
    const userRes = await fetch("https://api.github.com/user", {
        headers: { "Authorization": `Bearer ${tokenData.access_token}`, "User-Agent": "citum-hub-bun" }
    });
    ...
    return c.redirect(`${process.env.FRONTEND_URL || "http://localhost:3000"}/auth/callback?token=${jwt}`);
});
    ...
    websocket: {
        message(ws: any, message: string) {
            try {
                ...
                ws.send(JSON.stringify({ type: "preview_result", html: "WS preview not yet fixture-linked" }));
