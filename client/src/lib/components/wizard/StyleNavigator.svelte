<script lang="ts">
	...
	async function loadChoicePreviews() {
		...
		for (const choice of currentAxis.choices) {
			const preview = await wizardStore.previewStyleWithUpdates(
				getAxisChoiceUpdates(currentAxis.id, choice.value)
			);
			nextPreviews[choiceKey(currentAxis.id, choice)] = getPreviewHtmlForAxis(
				wizardStore.family,
				currentAxis.id,
				preview
			);
