        ...
        <div class="flex flex-col gap-8">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="text-slate-500 text-sm">Last updated {new Date(style.updated_at).toLocaleDateString()}</p>
                </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-8 relative overflow-hidden">
                ...
                <ComprehensivePreview previewSet={previewSet} />
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 flex flex-col gap-8">
                    <section class="bg-white rounded-3xl border border-slate-200 p-8">
                        ...
                        <p class="text-slate-600 leading-relaxed mb-6">
                            {style.description || `Official citation style for ${style.title}. Fully compliant with Citum requirements.`}
                        </p>
                        
                        {#if style.fields?.length}
                            <div class="flex flex-wrap gap-2">
                                {#each style.fields as field}
                                    <a href="/?searchQuery={field}" class="bg-slate-50 text-slate-500 border border-slate-100 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider hover:bg-slate-100 hover:text-primary transition-colors">{field}</a>
                                {/each}
                            </div>
                        {/if}
                    </section>
                    ...
                    <section class="bg-slate-900 rounded-3xl p-8 text-white relative overflow-hidden group">
                        ...
                        {#if showSource}
                            <pre class="bg-black/30 p-6 rounded-2xl text-xs font-mono overflow-x-auto border border-white/5 text-slate-300 max-h-[600px]">{style.citum || JSON.stringify(style.intent, null, 2)}</pre>
                        {:else}
                            <div class="flex items-center justify-center py-10 opacity-40 italic text-sm text-slate-400">
                ...
                <aside class="flex flex-col gap-6">
                    <div class="bg-white rounded-3xl border border-slate-200 p-6">
                        <h3 class="text-sm font-black uppercase tracking-widest text-slate-400 mb-4">Original Authors</h3>
                        <div class="flex flex-col gap-4">
                            {#if style.source?.original_authors?.length}
                                {#each style.source.original_authors as author}
                                    <div class="flex items-center gap-3">
                                        <div class="size-8 bg-slate-100 rounded-full flex items-center justify-center text-slate-400">
