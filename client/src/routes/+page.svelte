...
<main class="min-h-screen bg-slate-50">
	<section class="bg-slate-900 pt-24 pb-20 px-4">
		<div class="max-w-[1200px] mx-auto text-center">
			...
			<div class="max-w-2xl mx-auto relative group">
				<span
					class="material-symbols-outlined absolute left-6 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-primary transition-colors"
					>search</span
				>
				<input
					type="text"
	...
	<section class="max-w-[1200px] mx-auto py-16 px-4">
		...
		{:else}
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
				{#each filteredStyles as style}
					<a
						href="/style/{style.id}"
						class="group bg-white rounded-[32px] border border-slate-200/60 p-8 hover:border-primary/30 hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-1 transition-all flex flex-col justify-between"
					>
						<div>
							<div class="flex items-center gap-3 mb-6">
								<div
									class="size-12 bg-slate-50 rounded-2xl flex items-center justify-center group-hover:bg-primary/5 transition-colors"
								>
									<span
										class="material-symbols-outlined text-2xl text-slate-400 group-hover:text-primary"
										>description</span
									>
								</div>
							...
							{#if style.fields?.length}
								<div class="flex flex-wrap gap-2 mb-4">
									{#each style.fields as field}
										<span
											class="text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-0.5 rounded-full border border-slate-100 uppercase tracking-tighter"
											>{field}</span
										>
									{/each}
								</div>
						...
						<div class="flex items-center justify-between pt-6 border-t border-slate-50">
							<span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest"
								>Updated {new Date(style.updated_at).toLocaleDateString()}</span
							>
							<span
		...
		<div
			class="mt-20 text-center bg-white rounded-[40px] border border-slate-200/60 p-12 lg:p-20 shadow-sm"
		>
			...
			<p class="text-slate-500 text-lg mb-10 max-w-xl mx-auto">
				Build your own custom style in minutes using our guided visual wizard.
			</p>
			<a
				href="/create-wizard"
				class="bg-primary text-white px-8 py-4 rounded-2xl font-bold text-lg hover:bg-primary-dark transition-all shadow-lg shadow-primary/20 inline-flex items-center gap-3"
			>
