import { Z as attr_class, X as escape_html, S as attr, Y as ensure_array_like, _ as stringify, $ as html, a0 as attr_style } from './exports-B4Nj0tM2.js';
import './client-qzoUCy8n.js';
import { w as wizardStore, g as getComponentLabel, a as getComponentType } from './wizard.svelte-BuKwCymi.js';
...
function ComponentEditor($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		const FIELD_OPTIONS = [
			...
			{
				value: "container-title-short",
				label: "Container Title Short"
			},
			{
				value: "authority",
				label: "Authority"
								...
								if (wizardStore.selectedComponent.componentType === "group") {
									...
									$$renderer.push(` `);
									{
										...
										$$renderer.select({
											...
											$$renderer.push(`<!--[-->`);
											const each_array_1 = ensure_array_like(FIELD_OPTIONS);
									...
									$$renderer.push(`<!--]--> <button type="button" class="rounded bg-primary px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-blue-700">Insert After</button></div></div></div>`);
								} else {
									$$renderer.push("<!--[!-->");
...
//# sourceMappingURL=_page.svelte-BJh6YGbX.js.map
